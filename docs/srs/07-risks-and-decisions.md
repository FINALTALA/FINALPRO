...
| BDR-016 | **✅ Approved** Branch-rejection rejects the whole vendor application | Sprint 3 (BL-VEND-003) needed a concrete rejection-consequence policy to implement FR-VEND-003's `reject` decision, and OPEN-005 left it undecided. Engineering's implementation (a `reject` decision on any one physical branch transitions the vendor `UNDER_REVIEW` → `REJECTED` unconditionally, with no per-branch partial state) was confirmed as the intended policy by the product owner on review of the implementing commit, rather than assumed unilaterally — formally accepted 2026-09-17. **Explicitly does not decide** whether/how a vendor may submit a new application after a rejection — the product owner flagged that as a separate, still-open decision (⚠ OPEN-010), not something to fold into this approval | Part 3, BR-026; emergent from the Sprint 3 review process (same pattern as BDR-015), not from a Part 0 Q-answer |
