...
| OPEN-006 | Sign-off on the FYP Delivery Increment slice (Part 1, D.4) | Part 0 (Q14) | **Proceeding without objection** across the Part 1–6 review cycle, but no explicit product-owner sign-off has been given — treated as still formally open rather than assumed closed; recommend an explicit confirmation before Part 8's sprint plan is executed against it |
...
| BDR-015 | **Proposed** FYP Delivery Increment as the build-scope reconciliation — **not yet formally accepted** | The full confirmed scope (BDR-001–013) is documented as the target design; a narrower, explicitly-scoped increment is *proposed* as what the 2-person/3-month team actually builds first. This remains a proposal, not a decision, until the product owner explicitly signs off (OPEN-006) — status kept consistent with OPEN-006 rather than presumed closed by the absence of objection | Part 1, D.4 — status tracked as OPEN-006 above (proceeding without objection during review, but formal sign-off is still required before Part 8's sprint plan commits to it) |
