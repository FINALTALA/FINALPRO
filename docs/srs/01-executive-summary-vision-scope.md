...
**How product aggregation and price comparison work.** Vendors submit offers manually, via CSV/Excel import, or through a vendor-authorized API/feed integration where one exists. Where an offer corresponds to an existing canonical product, it is matched automatically when an exact identifier (barcode/GTIN/MPN) is present, otherwise queued for human review before it is linked and published. Offers that cannot be reliably matched to any canonical product — handmade goods, local products, bundles, and other unique listings — are **not** forced onto an unrelated canonical product; they are published as standalone, vendor-authored listings under their own identity, searchable and purchasable like any other offer but outside the like-for-like comparison view. Comparison and search operate on canonical products: every matched vendor offer participates in a normalized "best offer" indicator that accounts for price (converted to a common comparison currency where vendors price in different currencies — see OPEN-007 for how this interacts with checkout), stock, delivery/pickup option, and store distance.
...
| **Vendor verification reviewer** | Reviews vendor-submitted physical-branch verification evidence (geolocation pin + storefront photo) during onboarding (BR-VENDOR-VERIFICATION) | Approve/reject/request-resubmission of verification evidence | Cannot edit vendor catalog, orders, or financials; cannot approve a branch with no evidence attached | Onboarding/verification evidence + minimum vendor-identity fields needed to verify it | Branch verification approval workflow; every decision produces an audit record (resolves OPEN-005) |
...
### C.1 Compact role-permission matrix

`F` = Full control within own scope (create/edit/delete) · `R` = Read-only · `A` = Approve/moderate (a review decision, not open-ended CRUD) · `S` = Submit-only (can create/attach, cannot approve own submission) · `–` = No access. Roles are grouped for readability (e.g. all vendor-staff sub-roles share one column, all onboarding/matching reviewers share one column); this will be expanded to the full per-sub-role, per-endpoint matrix in Part 4 (Section H) once every FR module's actions are enumerated in Part 2.

| Module | Guest | Customer | Vendor Owner/Admin | Vendor Staff¹ | Delivery Driver | Platform Admin² | Matching/Verification Reviewer | Content Moderator | Support Agent | Finance | Marketing |
|---|---|---|---|---|---|---|---|---|---|---|---|
| Catalog & taxonomy (categories/brands/attributes) | R | R | R | R | – | F | R | – | R | – | R |
| Canonical-product matching decisions | – | – | – (report only) | – | – | F | A | – | – | – | – |
| Vendor profile & branches | R | R | F (own) | F (own, scoped) | – | F | R | – | R | R | R |
| Vendor verification evidence (geo pin + photo) | – | – | S (own) | – | – | F | A | – | – | – | – |
| Product offers & inventory | R | R | F (own) | F (own, scoped) | – | F | R | – | R | – | R |
| Cart | R (browse only, no checkout) | F (own) | – | – | – | R | – | – | R | – | – |
| Orders & vendor suborders | – | F (own) | F (own suborders) | F (own suborders, scoped) | F (assigned, status/proof only) | F | – | – | F (within authorization limits) | R | – |
| Payments & subscriptions | – | R (own) | R (own) | – | – | F | – | – | R (limited refund auth) | F | – |
| Delivery & fulfillment | – | R (own) | F (own) | F (own, scoped) | F (assigned only) | F | – | – | R | – | – |
| Reviews | R | F (own, verified purchase) | R + respond | – | – | F | – | A | R | – | R |
| Support tickets | – | F (own) | F (own) | – | – | F | – | – | F (assigned/queue) | – | – |
| Analytics & reports | – | – | R (own store) | – | – | F | – | – | R | R | R |
| Platform configuration / RBAC | – | – | – | – | – | F | – | – | – | – | – |
...
| 4 | Payment | Yes — COD + online (confirmed Q3; online gateway pending OPEN-001; mixed-currency parent-order settlement pending OPEN-007) |
...
- **Notifications:** BR-DELIVERY-CONFIRM's three notifications are implemented against a real low-cost SMS provider if OPEN-004 resolves in time, otherwise logged/visible in an admin/dev view as a fallback — this satisfies the FYP exit criterion (three recorded, viewable notification attempts) but must **not** be described as meeting the production "externally delivered SMS" requirement, which stays gated on OPEN-004.
